...
9. **🤖 自然语言支持（LLM）**：不必记忆指令，直接 @机器人 说「查下我的奖杯」「群里谁最肝」「看看 @某人 在玩什么」「和 @某人 对比一下」即可触发（需在 AstrBot 中配置并启用 LLM 函数调用）。
...
除了上面的指令，本插件还通过 AstrBot 的 **函数调用（function-calling / llm_tool）** 能力注册了一组工具。只要机器人配置了可用的 LLM，你就可以用日常说话的方式使用：
...
LLM 会自动选择对应的工具并调用，结果与指令完全一致。被 @ 的人会被自动识别为查询目标（同样要求 TA 已绑定）。

> 提示：自然语言触发依赖 LLM 对意图的判断；若识别不准，直接使用 `/psn` 等指令最稳妥。绑定/解绑等操作仍建议使用指令完成。
